package com.codegym.furama_resort_spring.security;
...
//    auth.inMemoryAuthentication().passwordEncoder(NoOpPasswordEncoder.getInstance()).withUser("sena").password("123456").roles("USER");
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.authorizeRequests().antMatchers("/customer/**").access("hasRole('ROLE_ADMIN') or hasRole('ROLE_USER')");
        http.authorizeRequests().antMatchers("/resort-service/**").access("hasRole('ROLE_ADMIN') or hasRole('ROLE_USER')");
        http.authorizeRequests().antMatchers("/contract/**").access("hasRole('ROLE_ADMIN') or hasRole('ROLE_USER')");
        // Chỉ cho phép user có quyền ADMIN hoặc USER truy cập đường dẫn /user/**
        http.authorizeRequests().antMatchers("/employee/list").access("hasRole('ROLE_ADMIN')");
        ...
        http.authorizeRequests().and().formLogin()//
                ...
                .and().logout().logoutUrl("/doLogout").logoutSuccessUrl("/login?message=logout");
    }
}